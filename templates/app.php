...
<aside class="sidebar bg-white border-end">
    ...
    <nav class="side-nav nav nav-pills flex-column" aria-label="<?= h(t('nav.dashboard')) ?>">
        <?php foreach ($navItems as $navPage => $label): ?>
            <a class="nav-link <?= $page === $navPage ? 'active' : '' ?>" href="/?page=<?= h($navPage) ?>"><?= h($label) ?></a>
        <?php endforeach; ?>
    </nav>
</aside>
<div class="app-main">
    <header class="topbar navbar bg-white border-bottom sticky-top">
        <div><h1 class="h4 mb-0"><?= h($navItems[$page]) ?></h1></div>
    ...
    <main class="layout container-fluid">
        ...
        <?php if ($page === 'dashboard'): ?>
            <section class="metrics row g-3 mb-4">
                <div class="col-12 col-sm-6 col-xl-3"><div class="card metric-card"><div class="card-body"><strong><?= count($servers) ?></strong><span><?= h(t('dashboard.metric_servers')) ?></span></div></div></div>
                <div class="col-12 col-sm-6 col-xl-3"><div class="card metric-card"><div class="card-body"><strong><?= count($domains) ?></strong><span><?= h(t('dashboard.metric_domains')) ?></span></div></div></div>
                <div class="col-12 col-sm-6 col-xl-3"><div class="card metric-card"><div class="card-body"><strong><?= count($packages) ?></strong><span><?= h(t('dashboard.metric_packages')) ?></span></div></div></div>
                <div class="col-12 col-sm-6 col-xl-3"><div class="card metric-card"><div class="card-body"><strong><?= count($actions) ?></strong><span><?= h(t('dashboard.metric_actions')) ?></span></div></div></div>
            </section>
        ...
        <?php elseif ($page === 'domains'): ?>
            <section class="page-section card"><div class="card-body">
                <div class="section-head"><h2 class="h5 mb-0"><?= h(t('domains.title')) ?></h2><form method="post"><input type="hidden" name="_action" value="import_domains"><input type="hidden" name="_return" value="<?= h($returnPath) ?>"><button class="btn btn-primary"><?= h(t('domains.import')) ?></button></form></div>
                <div class="table-responsive mt-3"><table class="table table-hover align-middle"><thead><tr><th><?= h(t('domains.domain')) ?></th><th><?= h(t('domains.server')) ?></th><th><?= h(t('domains.owner')) ?></th><th><?= h(t('domains.registered')) ?></th><th><?= h(t('domains.next_billing')) ?></th><th><?= h(t('domains.registrar')) ?></th><th><?= h(t('domains.deletion')) ?></th><th><?= h(t('domains.subdomains')) ?></th><th></th></tr></thead><tbody>
                    <?php foreach ($domains as $domain): ?>
                        <tr class="domain-row <?= h(domain_row_class($domain)) ?>" data-domain-id="<?= (int)$domain['id'] ?>" data-domain-name="<?= h($domain['domain']) ?>">
                            <td><?= h($domain['domain']) ?></td><td><?= h($domain['server_name']) ?></td><td><?= h($domain['owner_name'] ?: ($domain['owner_external_id'] ? 'User #' . $domain['owner_external_id'] : '')) ?></td>
                            ...
                            <td><input class="form-control" name="registrar" value="<?= h($domain['registrar']) ?>"></td><td class="domain-status-cell"><?= domain_status_html($domain) ?></td>
